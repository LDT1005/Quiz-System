...
#include <fstream>
#include <string.h>
#include <time.h> // Bổ sung lại để tính thời gian thực tế chính xác
...
struct ThiSinh {
    ...
    int soCauDung;
    float diem;
    long tongThoiGian; // Dùng long để lưu số giây thực tế
};
...
void thiTracNghiem(CauHoi* nganHang, int tongSoCau);
void ghiLichSuThi(ThiSinh ts, const char* tenFile);
void inBangLichSu(const char* tenFile);
...
void thiTracNghiem(CauHoi* nganHang, int tongSoCau) {
    ...
    cout << "Ho va ten: "; cin.getline(ts.hoTen, 50);
    cout << "MSSV: "; cin.getline(ts.mssv, 20);
    cout << "Thoi diem bat dau (VD: Sang Ca 1): "; cin.getline(ts.thoiDiemBatDau, 20);
